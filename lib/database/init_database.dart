  ...
  initDatabase() async {
    try {
      ...
      db = await openDatabase("historico.db", version: 1, onCreate: (Database db, int version) async {
        await db.execute("CREATE TABLE historico (id INTEGER PRIMARY KEY, teamWinner TEXT, teamOne TEXT, teamTwo TEXT, ptsTeamOne int, ptsTeamTwo int, dataPartida TEXT)");
        print('Create');
      });
    } catch (e) {
      print(e);
